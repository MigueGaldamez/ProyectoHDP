...
class ReporteForm(forms.ModelForm):
	...
	cantidadPruebas = forms.IntegerField(min_value=0,error_messages={'required': requerido +"Pruebas realizadas",'min_value':'Ingrese # Positivo'},widget=forms.NumberInput(attrs={'placeholder':'# Pruebas','class': 'form-control'}))
	cantidadPositivas = forms.IntegerField(min_value=0,error_messages={'required': requerido+"Positivas",'min_value':'Ingrese # Positivo'},widget=forms.NumberInput(attrs={'placeholder':'# Positivos','class': 'form-control'}))
	sospechosos = forms.IntegerField(min_value=0,error_messages={'required': requerido+"Sospechosos",'min_value':'Ingrese # Positivo'},widget=forms.NumberInput(attrs={'placeholder':'# Sospechozos','class': 'form-control'}))
	...
	cantFemenino = forms.IntegerField(min_value=0,error_messages={'required': requerido+"Femnenino",'min_value':'Ingrese # Positivo'},widget=forms.NumberInput(attrs={'placeholder':'Cant. F.','class': 'form-control'}))
	cantMasculino = forms.IntegerField(min_value=0,error_messages={'required': requerido+"Masculino",'min_value':'Ingrese # Positivo'},widget=forms.NumberInput(attrs={'placeholder':'Cant. M.','class': 'form-control'}))
	edadCero = forms.IntegerField(min_value=0,error_messages={'required': requerido+"0-9",'min_value':'Ingrese # Positivo'},widget=forms.NumberInput(attrs={'placeholder':'# 0-9','class': 'form-control'}))
	edadDiez = forms.IntegerField(min_value=0,error_messages={'required': requerido+"10-19",'min_value':'Ingrese # Positivo'},widget=forms.NumberInput(attrs={'placeholder':'# 10-19','class': 'form-control'}))
	edadVeinte =  forms.IntegerField(min_value=0,error_messages={'required': requerido+"20-39",'min_value':'Ingrese # Positivo'},widget=forms.NumberInput(attrs={'placeholder':'# 20-39','class': 'form-control'}))
	edadCuarenta = forms.IntegerField(min_value=0,error_messages={'required': requerido+"40-59",'min_value':'Ingrese # Positivo'},widget=forms.NumberInput(attrs={'placeholder':'# 40-59','class': 'form-control'}))
	edadSesenta = forms.IntegerField(min_value=0,error_messages={'required': requerido+"60-79",'min_value':'Ingrese # Positivo'},widget=forms.NumberInput(attrs={'placeholder':'# 60 -79','class': 'form-control'}))
	edadOchenta = forms.IntegerField(min_value=0,error_messages={'required': requerido+"Mayores de 80",'min_value':'Ingrese # Positivo'},widget=forms.NumberInput(attrs={'placeholder':'# > 80','class': 'form-control'}))
	complemento = forms.CharField(error_messages={'required': requerido+"Complemento de direccion"},widget=forms.TextInput(attrs={'placeholder':'Complemento de direccion','class': 'form-control'}))
	estado = forms.IntegerField(required=False)
	eliminado = forms.IntegerField(required=False)
	duiIngresado = forms.CharField(max_length=9,min_length=9,widget=forms.TextInput(attrs={'placeholder':'DUI','class': 'form-control'}))
	...
	def clean_eliminado(self):
		...
		eliminado=0
		return eliminado
	def clean_duiIngresado(self):
		duiIngresado = self.cleaned_data.get('duiIngresado')
		try:
			if duiIngresado.isdigit():
				valor = False
			else:
				raise ValidationError(_('%(value)s ,No es un DUI valido'),params={'value': duiIngresado},)
			ultimo= duiIngresado[8]
			contador=9
			suma=0
			valor = False
			for numero in duiIngresado: #summador de los primeros 8 numeros del dui
...
class ReporteForm_actualizar(forms.ModelForm):
	departamento = forms.ModelChoiceField(queryset=Departamento.objects.all(), required = False)
	#municipio = forms.ModelChoiceField()
	departamento.empty_label="Seleccione"
	requerido = "Ingrese un Dato: "
	...
	class Meta:
		model = Reporte
		fields =['perfil','estado','cantidadPruebas','cantidadPositivas','sospechosos','fechaTomada','fechaEditado','cantFemenino','cantMasculino','edadCero','edadDiez','edadVeinte','edadCuarenta','edadSesenta','edadOchenta','municipio','complemento','departamento','eliminado']
	...
	def clean_estado(self):
		...
		try:
			if estado !=0 and estado !=1 and estado != 2:
				raise ValidationError(_(' Error en el estado, recarga la pagina si es necesario'))
		except(ValueError,TypeError):
			pass
		return estado
